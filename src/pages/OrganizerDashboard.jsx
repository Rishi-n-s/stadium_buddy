...
import { AlertTriangle, TrendingUp, TrendingDown, Utensils, Shield, Wifi, Bot, Send, AlertCircle } from 'lucide-react';
    ...
    <div className="w-full max-w-7xl mx-auto p-4 md:px-gutter mt-8 space-y-12 pb-24 min-h-screen">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-outline-variant/30 pb-6 mb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-surface border border-outline-variant p-1 flex items-center justify-center overflow-hidden">
            <img 
        ...
        <Badge variant="info" className="px-4 py-2 border border-secondary-fixed">
          <span className="w-2 h-2 bg-secondary-fixed rounded-full animate-pulse mr-2"></span>
          LIVE TELEMETRY: EN
        </Badge>
      </header>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        <div>
          ...
          <div className="flex gap-4 flex-wrap">
            <Badge variant="neutral" className="px-4 py-2 border border-outline-variant">
              <span className="w-2.5 h-2.5 rounded-full bg-secondary-fixed mr-2"></span>
              {totalAttendees.toLocaleString()} ATTENDEES
            </Badge>
            <Badge variant="neutral" className="px-4 py-2 border border-outline-variant">
              <span className="w-2.5 h-2.5 rounded-full bg-primary mr-2 shadow-[0_0_8px_#0047fa]"></span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-8">
        ...
        <Card className="md:col-span-8 p-6 md:p-8 flex flex-col justify-between relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-br from-error/10 to-transparent pointer-events-none" />
          
          <div className="relative z-10">
            <div className="flex justify-between items-start">
              <Badge variant="error" className="px-4 py-1 mb-4">
                ZONE NORTH GATE
              </Badge>
              <AlertTriangle className="text-error animate-pulse h-8 w-8" />
            </div>
            <h3 className="text-headline-lg font-headline-lg italic uppercase mt-2 text-on-surface">High Congestion Warning</h3>
        ...
        <Card className="md:col-span-4 p-6 flex flex-col justify-between">
          <div>
            <Badge variant="neutral" className="px-4 py-1 mb-6">
            ...
            <div className="space-y-4 font-mono text-sm">
              ...
              <div className="flex justify-between items-center pb-2">
                ...
                <span className={`font-bold flex items-center gap-1 ${zones.east_stand.trend > 0 ? 'text-secondary-fixed' : 'text-primary'}`}>
                  {zones.east_stand.trend > 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <Card className="lg:col-span-8 p-6 flex flex-col h-[70vh] min-h-[400px] lg:h-[600px] relative overflow-hidden">
           <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
              <Bot className="h-32 w-32" />
           </div>
          <div className="flex justify-between items-start mb-6 relative z-10">
            <Badge variant="info" className="px-4 py-2 border border-primary">
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          ...
          <Card className="p-6 relative overflow-hidden">
            ...
            {activeAdvisories.length > 0 ? (
              <div className="space-y-6">
                {activeAdvisories.map((adv) => (
                  <div key={adv.id} className="border-b border-outline-variant/30 pb-4 last:border-b-0 last:pb-0">
                    ...
                    <p className="text-body-md font-body-md text-on-surface mb-4">{adv.text}</p>
                    <Button 
              ...
              <p className="text-body-md font-body-md text-on-surface-variant italic">All stadium operations running optimal. No alerts active.</p>
            )}
          </Card>

          {/* Live Dispatch GPS Map */}
          <Card className="p-1 flex flex-col h-auto min-h-[300px] overflow-hidden">
